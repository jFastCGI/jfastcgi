...
public class FastCGIHandlerFactory {
	...
	public final static String[] PARAM_NAMES = new String[] { PARAM_SERVER_ADDRESS, PARAM_START_EXECUTABLE, PARAM_CONNECTION_FACTORY, PARAM_CLUSTER_ADRESSES, PARAM_FILTERED_HEADERS, PARAM_KEEP_ALIVE };
	...
	public static FastCGIHandler create(final Map<String, String> config) {
		...
		if (config.get(PARAM_SERVER_ADDRESS) != null) {
			...
			final String className = config.get(PARAM_CONNECTION_FACTORY).trim();
			...
			final PoolFactory poolFactory = PoolFactoryFactory.getOrCreatePoolFactory();
			for (final String addr : config.get(PARAM_CLUSTER_ADRESSES).replaceAll("[\n\t]", ";").replaceAll(" ", "").split(";")) {
				if (!addr.isEmpty()) {
					getLog().info("  => " + addr);
		...
		if (config.get(PARAM_FILTERED_HEADERS) != null) {
			final String[] filteredHeaders = config.get(PARAM_FILTERED_HEADERS).split(";");
	...
	private static ConnectionFactory buildConnectionFactoryForClass(final String className) {
		try {

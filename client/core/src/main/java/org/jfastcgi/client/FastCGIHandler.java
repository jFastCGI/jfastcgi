...
public class FastCGIHandler {
    ...
    private static final Logger LOGGER = LoggerFactory
            .getLogger(FastCGIHandler.class);
    ...
    public void setFilteredHeaders(final String[] filteredHeaders) {
        ...
        if (filteredHeaders.length > 0) {
            ...
            final Pattern pattern = Pattern.compile(
                    regex.toString().substring(1), Pattern.CASE_INSENSITIVE);
    ...
    public void startProcess(final String cmd) throws IOException {
        ...
        processExecutor.setStreamHandler(new ExecuteStreamHandler() {
            ...
            public void setProcessOutputStream(final InputStream is)
                    throws IOException {
                loggers.add(new StreamLogger(is, LoggerFactory
                        .getLogger(FastCGIHandler.class.getName()
                                + ".externalprocess.stdout")));
            }
        ...
        pe.execute(CommandLine.parse(cmd), new DefaultExecuteResultHandler() {
            ...
            public void onProcessComplete(final int exitValue) {
                getLog().info(
                        String.format(
                                "external process exited with code %s : %s",
                                exitValue, cmd));
            }
        });
    ...
    public void service(final RequestAdapter request,
            final ResponseAdapter response) throws IOException {
    ...
    private boolean handleRequest(final RequestAdapter req,
            final ResponseAdapter res, final ISocket fcgiSocket,
    ...
    private void setEnvironment(final OutputStream ws, final RequestAdapter req)
            throws IOException {
    ...
    private int parseHeaders(final ResponseAdapter res, final InputStream is)
            throws IOException {
        String key = "";
        String value = "";
    ...
    private void addHeader(final OutputStream ws, final String key,
    ...
    private void writeHeader(final OutputStream ws, final int type,
    ...
    static class FastCGIInputStream extends InputStream {
            ...
            while (_is.read() >= 0) {
                ...
                case FCGI_STDOUT:
                    if (getLog().isDebugEnabled()) {
                        getLog().debug(
                                _fcgiSocket + ": FCGI_STDOUT(length:" + length
                                + ", padding:" + padding + ")");
                    }
                ...
                case FCGI_STDERR:
                    if (getLog().isDebugEnabled()) {
                        getLog().debug(
                                _fcgiSocket + ": FCGI_STDERR(length:" + length
                                + ", padding:" + padding + ")");
                    }
                ...
                default:
                    getLog().warn(
                            _fcgiSocket + ": Unknown Protocol(" + type + ")");
        ...
        private boolean fcgi_end_request() throws IOException {
            final int appStatus = (_is.read() << 24) + (_is.read() << 16)
                    + (_is.read() << 8) + _is.read();
            final int pStatus = _is.read();

            if (getLog().isDebugEnabled()) {
                getLog().debug(
                        _fcgiSocket + ": FCGI_END_REQUEST(appStatus:"

...
public class FastCGIHandler {
	...
	/**
	 * Some http headers have sometimes to be filtered for security reasons, so this methods allows to tell which http headers we do not want to pass to the fastcgi app. For example : <code>
	 ...
	 *            an array of http header keys that will not be transmitted to the fastcgi responder app.
	 */
	...
	private boolean handleRequest(final RequestAdapter req, final ResponseAdapter res, final ISocket fcgiSocket, final OutputStream out, final boolean keepalive) throws IOException {
